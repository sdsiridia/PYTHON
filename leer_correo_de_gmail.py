...
# por algun  otivo lee siempre el mismo email con la opcion ("INBOX",readonly=False) deberia dejar pero no
...
for i in range(mensajes, mensajes - N, -1):
    ...
    #     # Obtener el mensaje
    try:  # -> por si la camntidad de mensajes que quiero leer es superior a la que existen
        # fetch -> obtener el mensaje i RFC822 -> que voy as pedir todas las partes del mensaje
        # las variables res y mensaje es por que 'imap.fetch(str(i), "(RFC822)")'
        # nos devuelve la respuesta de la solicitud y el mensaje
        # srt(i) -> i es el numero de mensaje
        res, mensaje = imap.fetch(str(i), "(RFC822)")
        # print(mensaje) -> para mostrar por consola los mensajes
        # está de una manera un poco ilejible
